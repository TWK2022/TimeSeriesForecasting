...
# 数据精度为float64
# 获取的数据保存为:股票名.csv。重复获取时，只会获取新的数据并补全原数据
# -------------------------------------------------------------------------------------------------------------------- #
# 测试
...
parser.add_argument('--end_time', default='2023-09-1', type=str, help='|结束时间，要小于当前日期的前一天|')
parser.add_argument('--max_data', default=2500, type=int, help='|wind函数每次的获取上限，将数据分为多次获取|')
